...
def fullCic(xe = xe,ye = ye,Xw = 0.0117,Xd = 0.8315,infoR01 = 1,sca01 = 1,limit = 0.1):
    ...
    fig.canvas.manager.set_window_title('精馏塔全回流理论板绘制'+time.strftime(".%Y%m%d%H%M%S", time.localtime()))
    ...
    if sca01 == 1:
        ...
        axins.set_yticklabels([str(limit)])
        if 0<=Xw<Xd<=limit or 0<=Xd<Xw<=limit:
            if Xw<(0.75*limit) and Xd<(0.75*limit):
                axins.set_xticks([Xw,Xd,limit])
                axins.set_xticklabels(['Xw','Xd',str(limit)])
            else:
                axins.set_xticks([Xw,Xd])
                axins.set_xticklabels(['Xw','Xd'])
        if 0<=Xw<=limit and Xd>limit :
            if Xw<=(0.75*limit):
                axins.set_xticks([Xw,limit])
                axins.set_xticklabels(['Xw',str(limit)])
            else:
                axins.set_xticks([Xw])
                axins.set_xticklabels(['Xw'])
        if 0<=Xd<=limit and Xw>limit:
            if Xd<=(0.75*limit):
                axins.set_xticks([Xd,limit])
                axins.set_xticklabels(['Xd',str(limit)])
            else:
                axins.set_xticks([Xd])
                axins.set_xticklabels(['Xd'])
        if Xw>limit and Xd>limit:
            axins.set_xticks([limit])
            axins.set_xticklabels([str(limit)])
...
# 标 1 为精馏段数据 ， 标 2 为提馏段数据
    ...
    #图像框架设置
    fig = plt.figure(figsize=(5.5, 3.5))
    fig.canvas.manager.set_window_title('精馏塔部分回流理论板绘制'+time.strftime(".%Y%m%d%H%M%S", time.localtime()))
    host = host_subplot(111, axes_class=axisartist.Axes)
    ax = plt.subplot(1,1,1)
    aspectratio=1.0
    ratio_default=(ax.get_xlim()[1]-ax.get_xlim()[0])/(ax.get_ylim()[1]-ax.get_ylim()[0])
...
def pump(Q = Q,H = H,N = N,η = η):
    ...
    fig = plt.figure(figsize=(5.5, 3.5))
    fig.canvas.manager.set_window_title('离心泵特性曲线绘制'+time.strftime(".%Y%m%d%H%M%S", time.localtime()))
        ...
        while 1 :
            ...
            if order == 'show' :
                if Xw == 'none' or Xd == 'none':
                    ...
                    if Xw>=Xd:
                        order = input(' ！！Xw大于或等于Xd，确定要进行绘制？(Y/N)！！')
                        if order == 'y' or order == 'Y' or order == '1':
